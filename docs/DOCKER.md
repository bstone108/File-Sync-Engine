...
The package in `web-gui/dist/` is now a small **read-only functional reference package**: it reads daemon-owned same-origin status, folders, and peers boundaries, shows node/state/folder/peer counts plus normal-user folder and peer summaries, and provides actionable loading errors without receiving the native API credential. It is deliberately not yet a complete server deployment interface: transfers, settings, and mutating controls are still pending. Use a separately versioned, trusted package for normal deployment; the core image remains package-free.

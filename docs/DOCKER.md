...
The package in `web-gui/dist/` is now a small **status-only functional reference package**: it reads the same-origin daemon-owned status boundary, shows node/state/folder/peer counts, and provides actionable status-loading errors without receiving the native API credential. It is deliberately not yet a complete server deployment interface: folders, peers, transfers, settings, and mutating controls are still pending. Use a separately versioned, trusted package for normal deployment; the core image remains package-free.

...
When an enabled package is served, its same-origin read-only `GET /api/v1/status`, `GET /api/v1/folders`, `GET /api/v1/peers`, `GET /api/v1/transfers`, and `GET /api/v1/actionable-errors` bridges invoke the daemon's protected matching `/v1/*` handlers in-process. The transfer bridge deliberately discards browser query parameters and uses the daemon's bounded default history limit. The actionable-errors endpoint is a compact allowlisted category/count/action summary of recent operational error events; it deliberately excludes event messages, filesystem paths, folder/peer IDs, and credentials, so administrators must review daemon logs on the server for diagnosis. The bridges accept no browser-supplied credential or headers, inject the native API credential only inside the daemon, and expose no other native API routes. The folder, peer, transfer, and error views are read-only; folder edits, peer/transfer actions, settings, and every mutating control require separately allowlisted, authorization-aware endpoints.

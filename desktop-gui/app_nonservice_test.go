...
func TestWriteGUIOwnedDaemonConfigUsesDaemonAPIKeySchema(t *testing.T) {
	tmp := t.TempDir()
	configPath := filepath.Join(tmp, "config.jsonc")
	const apiKey = "gui-owned-daemon-test-key"
	if err := writeGUIOwnedDaemonConfig(configPath, "127.0.0.1:22420", apiKey, tmp); err != nil {
		t.Fatalf("write GUI-owned daemon config: %v", err)
	}
	configBytes, err := os.ReadFile(configPath)
	if err != nil {
		t.Fatalf("read GUI-owned daemon config: %v", err)
	}
	configText := strings.Replace(string(configBytes), "// Generated by the desktop GUI for a separate GUI-owned non-service daemon session.\n", "", 1)
	var decoded struct {
		API struct {
			Key string `json:"key"`
		}
	}
	if err := json.Unmarshal([]byte(configText), &decoded); err != nil {
		t.Fatalf("decode GUI-owned daemon config: %v", err)
	}
	if decoded.API.Key != apiKey {
		t.Fatalf("daemon API key = %q, want the GUI-owned credential", decoded.API.Key)
	}
}

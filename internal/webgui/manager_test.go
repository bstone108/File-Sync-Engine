...
func TestServerProvidesSameOriginFoldersWithoutExposingNativeAPIKey(t *testing.T) {
	dir := t.TempDir()
	installDir := filepath.Join(dir, "web", "current")
	if err := os.MkdirAll(installDir, 0o755); err != nil {
		t.Fatal(err)
	...
	native := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		nativeCalls++
		if r.Method != http.MethodGet || r.URL.Path != "/v1/folders" {
			t.Fatalf("native request = %s %s, want GET /v1/folders", r.Method, r.URL.Path)
		}
		if got := r.Header.Get("X-FSE-API-Key"); got != "native-secret" {
			t.Fatalf("native API key = %q, want injected server-side key", got)
		}
		...
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`[{"id":"docs","mode":"sendrecv","status":"idle"}]`))
	})
	...
	req, err := http.NewRequest(http.MethodGet, status.URL+"/api/v1/folders", nil)
	if err != nil {
		t.Fatal(err)
	}
	req.Header.Set("X-FSE-API-Key", "browser-supplied-secret")
	...
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		t.Fatalf("same-origin folders request: %v", err)
	}
	body, err := io.ReadAll(resp.Body)
	_ = resp.Body.Close()
	if err != nil {
	...
	if resp.StatusCode != http.StatusOK || string(body) != `[{"id":"docs","mode":"sendrecv","status":"idle"}]` {
		t.Fatalf("same-origin folders response = %d %q", resp.StatusCode, body)
	}
	if nativeCalls != 1 {
		t.Fatalf("native calls = %d, want 1", nativeCalls)
	}

	blocked, err := http.Post(status.URL+"/api/v1/folders", "application/json", nil)
	if err != nil {
		t.Fatalf("blocked folders mutation request: %v", err)
	}
	_ = blocked.Body.Close()
	if blocked.StatusCode != http.StatusMethodNotAllowed || nativeCalls != 1 {
		t.Fatalf("folders bridge must remain GET-only: status=%d calls=%d", blocked.StatusCode, nativeCalls)
	}
}

func TestServerProvidesSameOriginPeersWithoutExposingNativeAPIKey(t *testing.T) {
	dir := t.TempDir()
	installDir := filepath.Join(dir, "web", "current")
	if err := os.MkdirAll(installDir, 0o755); err != nil {
		t.Fatal(err)
	...
	native := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method != http.MethodGet || r.URL.Path != "/v1/peers" {
			t.Fatalf("native request = %s %s, want GET /v1/peers", r.Method, r.URL.Path)
		}
		if got := r.Header.Get("X-FSE-API-Key"); got != "native-secret" {
			t.Fatalf("native API key = %q, want injected server-side key", got)
		}
		...
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`[{"id":"peer-a","status":"connected"}]`))
	})
	...
	req, err := http.NewRequest(http.MethodGet, status.URL+"/api/v1/peers", nil)
	if err != nil {
		t.Fatal(err)
	}
	req.Header.Set("X-FSE-API-Key", "browser-supplied-secret")
	...
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		t.Fatalf("same-origin peers request: %v", err)
	}
	body, err := io.ReadAll(resp.Body)
	_ = resp.Body.Close()
	if err != nil {
	...
	if resp.StatusCode != http.StatusOK || string(body) != `[{"id":"peer-a","status":"connected"}]` {
		t.Fatalf("same-origin peers response = %d %q", resp.StatusCode, body)
	}
	if nativeCalls != 1 {
		t.Fatalf("native calls = %d, want 1", nativeCalls)
	}

	blocked, err := http.Post(status.URL+"/api/v1/peers", "application/json", nil)
	if err != nil {
		t.Fatalf("blocked peers mutation request: %v", err)
	}
	_ = blocked.Body.Close()
	if blocked.StatusCode != http.StatusMethodNotAllowed || nativeCalls != 1 {
		t.Fatalf("peers bridge must remain GET-only: status=%d calls=%d", blocked.StatusCode, nativeCalls)
	}
}
...
func TestServerProvidesSameOriginActionableErrorsWithoutExposingNativeAPIKey(t *testing.T) {
	dir := t.TempDir()
	installDir := filepath.Join(dir, "web", "current")
	if err := os.MkdirAll(installDir, 0o755); err != nil {
		t.Fatal(err)
	...
	native := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		nativeCalls++
		if r.Method != http.MethodGet || r.URL.Path != "/v1/actionable-errors" || r.URL.RawQuery != "" {
			t.Fatalf("native request = %s %s?%s, want GET /v1/actionable-errors without browser query", r.Method, r.URL.Path, r.URL.RawQuery)
		}
		if got := r.Header.Get("X-FSE-API-Key"); got != "native-secret" {
			t.Fatalf("native API key = %q, want injected server-side key", got)
		}
		...
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"errors":[{"kind":"sync","action":"Review the daemon logs and folder access on the server.","count":1}]}`))
	})
	...
	req, err := http.NewRequest(http.MethodGet, status.URL+"/api/v1/actionable-errors?limit=100", nil)
	if err != nil {
		t.Fatal(err)
	}
	req.Header.Set("X-FSE-API-Key", "browser-supplied-secret")
	...
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		t.Fatalf("same-origin actionable errors request: %v", err)
	}
	body, err := io.ReadAll(resp.Body)
	_ = resp.Body.Close()
	if err != nil {
	...
	if resp.StatusCode != http.StatusOK || string(body) != `{"errors":[{"kind":"sync","action":"Review the daemon logs and folder access on the server.","count":1}]}` {
		t.Fatalf("same-origin actionable errors response = %d %q", resp.StatusCode, body)
	}
	if nativeCalls != 1 {
		t.Fatalf("native calls = %d, want 1", nativeCalls)
	}

	blocked, err := http.Post(status.URL+"/api/v1/actionable-errors", "application/json", nil)
	if err != nil {
		t.Fatalf("blocked actionable errors mutation request: %v", err)
	}
	_ = blocked.Body.Close()
	if blocked.StatusCode != http.StatusMethodNotAllowed || nativeCalls != 1 {
		t.Fatalf("actionable errors bridge must remain GET-only: status=%d calls=%d", blocked.StatusCode, nativeCalls)
	}
}
